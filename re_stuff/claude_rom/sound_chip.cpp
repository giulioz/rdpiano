...
// ============================================================================
// High-level voice/part API (direct SA_Part access, no write() overhead)
// ============================================================================


void SoundChip::setPitch(int voice, int part, uint16_t pitch) {
    m_parts[voice][part].pitch_lut_i = pitch;
}

void SoundChip::setWave(int voice, int part, uint8_t wave_loop, uint8_t wave_high) {
    m_parts[voice][part].wave_addr_loop = wave_loop;
    m_parts[voice][part].wave_addr_high = wave_high;
}

void SoundChip::setEnvelope(int voice, int part, uint8_t env_dest, uint8_t env_speed) {
    m_parts[voice][part].env_dest = env_dest;
    m_parts[voice][part].env_speed = env_speed;
}

void SoundChip::setFlags(int voice, int part, uint8_t flags) {
    // Flags are shared across all parts of a voice (stored on part 0)
    m_parts[voice][0].flags_0 = flags & 1;
    m_parts[voice][0].flags_1 = (flags >> 1) & 1;
}

void SoundChip::setEnvOffset(int voice, int part, uint8_t offset) {
    m_parts[voice][part].env_offset = offset;
}

void SoundChip::silencePart(int voice, int part) {
    setEnvelope(voice, part, 0, 0);
}
...
void SoundChip::clearPart(int voice, int part) {
    ...
    p.env_speed = 0;
    p.env_offset = 0xFF;
    // Flags on part 0 only
    if (part == 0) { p.flags_0 = false; p.flags_1 = false; }
}
